...
def new_task():
    ...
    # return HTML which displays a form to create a new task
    return render_template('tasks/new.html', all_users=users)

@tasks_blueprint.route('/tasks', methods=['POST'])
def create_task():
    # grab all the bits from the form assign to variables
    description = request.form["description"]
    user_id = request.form["user"]
    duration = request.form["duration"]
    completed = request.form["completed"]

    # find the right user from database based on the user id
...
def show_task(id):
    ...
    # find the right task in database by the id
